...
function initSocket(options, parent) {
  ...
  socket.processQueue = function () {
    ...
    for (var i in messages) {
      ...
      if (global.c['dangerous']) {
        dd(('[' + helpers.safeLog(message.packet.getRawMessage({lengthHeader: global.c['useLengthHeader']}), ['isoMessage']) + ']').gray, 'verbose');
      } else {
        dd(('[' + helpers.safeLog(message.packet.getRawMessage({lengthHeader: global.c['useLengthHeader']}), ['isoMessage']).substring(0,8) + '...]').gray, 'verbose');
      }     
  ...
  socket.on('data', function (data) {
    ...
    for (var index in packetData) {
      ...
      if (multiMessage) dd(('[' + helpers.safeLog(packetData[index], ['isoMessage']) + ']').gray, 'verbose');
