...
var uOptions = {
  ...
  // Delay before forced connection close
...
function initSocket(options, parent) {
  ...
  // Resetting upstream socket status variables
  ...
  socket.on('error', function (err) {
    socket.retryDelay = err == 'ECONNREFUSED' ? uOptions.timeoutErrorRefused : uOptions.timeoutError;

    dd(('Upstream socket error ' + err).red);
    this.end();
  });

  // 'Timeout' event is triggered on connection timeout (no data sent or received for a given period of time)
  socket.on('timeout', function () {
    ...
    socket.terminateTimer = setTimeout(function() {
      ...
      this.destroy();
    }.bind(socket), uOptions.timeoutSocketTerminate);
