...
var clientHttpServer = require('./clientHttpServer');
    ...
    connection.on('data', function (data) {
      ...
      for (var index in packetData) {
        ...
        if (!p.parseError) {
          ...
          if (!qe) {
            if (options.downstreamWebhookURL) {
              inboundRequestHandle(p, options, socket)
            } else {
...
function inboundRequestHandle(packet, options, socket, retry) {
  if (!retry) {
    retry = 1
  }

  var url = options.downstreamWebhookURL
  var message = clientHttpServer.prepareObjectDataOut(packet.getFields())
  var body = {
    result: message,
    errors: [],
  }
  ...
  if (retry > 5) {
    dd(('Downstream webhook request: max request retries reached').red)
    dd(packet.pretty(), 'verbose')
    return
  }

  dd((`Downstream webhook request: try #${retry} to send request`).green, 'verbose')
  axios
    .post(url, body)
    .then(function (response) {
      var message = clientHttpServer.prepareObjectDataIn(response.data)
      socket.parent.sendData(socket.upstreamConnection, message)
    })
    .catch(function (error) {
      dd((`Downstream webhook request: error ${error}`).red)
      setTimeout(function () {
        inboundRequestHandle(packet, options, socket, ++retry)
