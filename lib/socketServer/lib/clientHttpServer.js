...
function clientHttpServer(upstream, c) {
  ...
  var server = http.createServer(function (req, res) {
    ...
    req.reply = function(p) {
      if (!this.isRawMode) {
        var msg = prepareDataOut(p.getFields());
      } else {
...
function preparedataectDataIn(data) {
  if (data.hasOwnProperty(55)) {
    data[55] = new Buffer(data[55], "hex");
  }
...
function prepareDataOut(data) {
  if (data.hasOwnProperty(55)) {
    if (typeof data[55] == 'dataect' && data[55] instanceof Buffer) {
      data[55] = helpers.safeLog(data[55].toString('hex'), ['field55']);
    } else {
      data[55] = helpers.safeLog((new Buffer(data[55], 'binary')).toString('hex'), ['field55']);
    }
  }
