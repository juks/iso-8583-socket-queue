...
function initSocket(options, parent) {
  ...
  var socket = net.createServer(function (connection) {
    ...
    connection.on('data', function (data) {
      parent.processHostData(socket, data, { stopOnUnmatchedPacket: true });
    });
    ...
    connection.on('error', function (err) {
      socket.retryDelay = err == 'ECONNREFUSED' ? UpstreamChannel.defaults.timeoutErrorRefused : UpstreamChannel.defaults.timeoutError;
