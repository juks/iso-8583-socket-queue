...
queue.prototype.releaseTerminal = function(terminalId, key) {
  ...
  if (key && this.busyTerminals.hasOwnProperty(terminalId) && this.busyTerminals[terminalId] != key) {
    ...
    // Sometimes we release the busyTerminals dict
    if (Object.keys(this.busyTerminals).length == 0) {
      if (helpers.randomCase(20)) {
        dd('Refreshing the busyTerminals object');
        this.busyTerminals = {};
      }
    }
...
queue.prototype.confirm = function(id) {
  ...
  if (item) {
    ...
    // Sometimes we release the queue object
...
queue.prototype.getActiveByPacket = function(p) {
  ...
  for (var i in this.q) {
    var qe = this.q[i];
    if (qe.stan == stan && qe.terminalId == tid && qe.isProcessed) {
