...
queue.prototype.queueTimeout = function(itemId) {
  ...
  if (!item) {
    dd('Queue notice: timeout for unknown item #' + itemId, 'verbose');
    return;
  }

  dd(('Queue notice: ' + item.senderName + ' reached queue timeout').yellow);
  if (!item.sender.isClosed()) {
    this.confirm(itemId);
...
// Returns messages, ready to process
...
// Checks of currently there is no message with given terminal id in active state
...
queue.prototype.releaseTerminal = function(terminalId, key) {
  ...
  // If terminal was taken by someone else -- do nothing
...
queue.prototype.getActiveByPacket = function(p) {
  ...
  for (var i in elements) {
    ...
    for (var fieldIdx in global.c['queueMessageKeyFields']) {
      var id = global.c['queueMessageKeyFields'][fieldIdx];
  ...
  if (pretenders.length == 1) {
    return pretenders[0];
  // We have more than one sender, pretending to be the recipient -- need to solve problem
