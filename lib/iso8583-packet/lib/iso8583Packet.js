...
function Iso8583Packet(data, options) {
  ...
  if (data) {
    ...
    if (typeof data == 'string' || typeof data == 'object' && data instanceof Buffer) {
      ...
      if (!options.hasOwnProperty('staticHeader') && !this.packager.config.validators.isValidMessage(data)) {
        this.parseError = 'Invalid message!';

        return null;
      } else if (options.hasOwnProperty('staticHeader') && !global.c['lengthHeaderFirst'] && options.staticHeader && options.staticHeader.length && !Buffer.compare(data.slice(0, options.staticHeader.length), new Buffer(options.staticHeader))) {
...
Iso8583Packet.prototype.pad = function() {
  for (var i in this.systemConfig.padFields) {
    if (this.packager.format.hasOwnProperty(this.systemConfig.padFields[i]) && this.packager.format[this.systemConfig.padFields[i]].hasOwnProperty('length')) {
      ...
      if (val != null) {
        val = this.zeroPad(val, this.systemConfig.padFields[i]);
        var update = {};
...
Iso8583Packet.prototype.getMessage = function(options) {
  ...
  if (global.c['lengthHeaderFirst']) {
    return Buffer.concat([lengthHeader, staticHeader, message]);
  } else {
...
Iso8583Packet.prototype.getRawMessage = function(options) {
  ...
  if (typeof this.rawData == 'object' && this.rawData instanceof Buffer) {
    if (global.c['lengthHeaderFirst']) {
      return Buffer.concat([lengthHeader, staticHeader, this.rawData]);
    } else {
      return Buffer.concat([staticHeader, lengthHeader, this.rawData]);
    }
  } else {
    if (global.c['lengthHeaderFirst']) {
      return lengthHeader.toString() + staticHeader.toString() + this.rawData;
    } else {
...
Iso8583Packet.prototype.getLengthHeader = function(lengthVal) {
  return this.packager.config.generators.getLengthHeader(lengthVal);
}
