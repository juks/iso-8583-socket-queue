...
Iso8583Packet.prototype.getFieldCaption = function(id, val) {
  ...
  // POS Entry Mode
  if (id == 22) {
    var part1 = val.toString().slice(0,2);
    var part2 = val.toString().slice(-1);
  ...
  // Response Code
    ...
    if (this.systemConfig.errors.hasOwnProperty(val.trim())) {
      result = this.systemConfig.errors[val.trim()];
    }
  }
...
Iso8583Packet.prototype.isValidError = function(code) {
  return this.systemConfig.errors.hasOwnProperty(code.trim());
};
