...
exports.generators = {
  getHeader: function(lengthVal) {
    var lengthHeaderLength = exports.processors.getLengthHeaderLength();
    var lengthHeaderType = exports.processors.getLengthHeaderType();

    var result = new Buffer('');

    exports.headerFormat.forEach(function(header) {
      if (header.name == 'length') {
        // Numeric header
        if (lengthHeaderType == 'fixed-n') {
          result = Buffer.concat([result, this.getLengthHeaderNumeric(lengthVal, lengthHeaderLength)]);
          // HEX header
        } else {
          result = Buffer.concat([result, this.getLengthHeaderHex(lengthVal, lengthHeaderLength)]);
        }
      } else {
      }
    }.bind(this));
...
exports.processors = {
  splitByHeader: function(message) {
    ...
    var lengthHeaderLength = this.getLengthHeaderLength();
    var lengthHeaderType = this.getLengthHeaderType();

    while (1) {
      if (lengthHeaderType == 'fixed-n') {
        length = this.readLengthHeaderNumeric(message, index, lengthHeaderLength);
      } else {
        length = this.readLengthHeaderHex(message, index, lengthHeaderLength);
      }

      if (!length) break;

      index += lengthHeaderLength;
  ...
  readLengthHeaderNumeric: function(message, position, length) {
    var cnt = message.slice(position, position + length);

    return parseInt(cnt.toString());
  },

  readLengthHeaderHex: function(message, position, length) {
    var cnt = message.slice(position, position + length);
